...
- [CPS _(continuation-passing style)_](https://en.wikipedia.org/wiki/Continuation-passing_style)：函数通过参数传递 **调用结果的处理逻辑**（即 continuation），而不是 让函数 **直接返回调用结果**（例如 [异步、非阻塞 I/O 模型](../2019/IO-Models.md) 常用回调函数 处理 I/O 结果；以及上文提到的 `signal`）
- [**策略模式** _(strategy pattern)_](../2017/Design-Patterns-Notes-3.md#Strategy)：函数通过参数传递 **中间状态的处理策略**，而不是 让函数 **内部写死处理策略**（例如 上文提到的 `qsort`）

## [no-number] 写在最后

本文仅是我对回调函数的一些理解。如果有什么问题，望不吝赐教。😄
