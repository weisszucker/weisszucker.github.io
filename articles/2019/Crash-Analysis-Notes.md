...
- 你可能看不到崩溃的位置的代码（例如第三方模块），需要用“逆向”思维来大胆假设
- 你需要掌握更全面的知识（了解 操作系统/编译原理/C++，熟悉被分析的代码），利用线索验证猜想
...
- 原因：
  - 野指针：释放后，再使用 或 重复释放，可能覆写正常对象内存
    - 异步回调时，在抛出任务时绑定的对象已被销毁
    - 同步调用时，调用栈上层函数 销毁 下层函数 还在使用（或将要使用）的对象
    - 循环内删除节点，导致迭代器失效：`for(auto it = c.begin(); it != c.end(); ++it) c.erase(it);`（一般是间接删除，例如 [通知时同步操作观察者列表](Insane-Observer-Pattern.md#问题：操作观察者列表)）
    - 存储裸指针 `std::vector<T*>`，而不是对象的弱引用 `std::vector<base::WeakPtr<T>>`
  - 缓冲区溢出：使用错误的 起始指针 或 缓冲区长度
    - 判断迭代器是否有效 `it != c.end()` 后，判断逻辑错误，继续使用非法迭代器 `c.end()`
  - 指针强转：基类指针 `static_cast<Derived*>(base)` 向下转成错误的派生类，对象布局不对齐（当前基类 恰好不是 派生类的第一个基类）
  - 数据竞争：
    - 其他线程 销毁 当前线程 正在使用的对象
      - `std::unique_ptr<>` 管理的对象被其他线程销毁，但当前线程正在通过裸指针使用
      - 可以改用 `std::shared_ptr<>`，使用前先将对象 PIN 到局部变量上
    - 多个线程 修改 非线程安全对象
      - UI 数据只允许 UI 线程访问
      - 多个线程重入字体缓存逻辑，导致字体缓存数据损坏
- 排查：
...
- 原因：
  - 前置条件改变，调用者对条件的假设（例如 `debug_assert`/`DCHECK`）失效
  - 内存破坏导致
- 排查：
  - 回溯调用栈，找到空指针首次出现的位置，倒推复现路径
  - 空指针加上 offset 再访问数据成员，变为野指针崩溃（特征：`ptr == offsetof(member)`）

### 除零崩溃

- 原因：
  - 整数除法 `/ 0`（浮点数除法 `/ 0` 返回 `NaN`，不会崩溃）
  - 取模 `% 0`
...
- 原因：
  - 逻辑缺陷导致无穷递归调用（例如 第三方持续发送 `WM_CLOSE`，弹出关闭确认对话框，阻塞代码）
  - 在栈上申请较大的局部变量，耗尽栈内存
- 排查：
  - 回溯调用栈，找到循环递归调用的范围
  - 通过 `ChildEBP` 分析栈帧的内存使用情况
...
- `main()` 前初始化（全局对象）
- 启动时初始化（消息循环栈上的对象）
- 消息循环/定时器阶段（局部对象 + 静态对象 + `LazySingleton`）
- 退出时反初始化（消息循环栈上的对象 + `AtExitManager`）
